...
class DrowsinessTCN(nn.Module):
    def __init__(self, input_size=4, num_channels=[32, 64, 64], kernel_size=3, dropout=0.2, num_classes=3):
        super(DrowsinessTCN, self).__init__()
...
def main():
    ...
    try:
        df = pd.read_csv('rldd_features.csv')
    ...
    print("2. TCN 딥러닝 모델 로드 중...")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = DrowsinessTCN(input_size=4, num_channels=[64, 64, 64], num_classes=3).to(device)
    
    try:
        model.load_state_dict(torch.load("tcn_drowsiness_model.pth", map_location=device))
        model.eval()
        print(" -> 모델 가중치 로드 완료!")
    except Exception as e:
        print(f"모델 가중치(tcn_drowsiness_model.pth)를 찾을 수 없습니다: {e}")
        return
    ...
    while cap.isOpened():
        ...
        cv2.imshow('TCN Drowsiness Inference', frame)
