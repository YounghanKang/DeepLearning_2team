...
class DrowsinessBiLSTM(nn.Module):
    def __init__(self, input_size=4, hidden_size=64, num_layers=2, num_classes=3):
        ...
        # 양방향이므로 hidden_size가 2배로 나옴 (정방향 + 역방향)
    ...
    def forward(self, x):
        ...
        # 가장 마지막 시점(seq_len)의 출력값을 Fully Connected Layer에 통과시킴
...
def main():
    ...
    model = DrowsinessBiLSTM(input_size=4, hidden_size=HIDDEN_SIZE, 
    ...
    save_path = os.path.join(BASE_DIR, "bilstm_drowsiness_model.pth")
    
    wandb.init(project="drowsiness-detection", name="Bi-LSTM")
    
    # 5. Training Loop
    print("\n--- 본격적인 Bi-LSTM 학습을 시작합니다 ---")
