...
def main():
    ...
    print("2. Bi-LSTM+Attention 딥러닝 모델 로드 중...")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = DrowsinessBiLSTMAttn(input_size=4, hidden_size=64, num_layers=2, num_classes=3).to(device)
    
    try:
        model.load_state_dict(torch.load(os.path.join(BASE_DIR, "bilstm_attn_drowsiness_model.pth"), map_location=device))
        model.eval()
        print(" -> 모델 가중치 로드 완료!")
    except Exception as e:
        print(f"모델 가중치(bilstm_attn_drowsiness_model.pth)를 찾을 수 없습니다: {e}")
        return
    ...
    while cap.isOpened():
        ...
        if detection_result.face_landmarks:
            ...
            # 4. 30프레임이 모이면 Bi-LSTM+Attention 추론
        ...
        cv2.imshow('Bi-LSTM+Attention Drowsiness Inference', frame)
