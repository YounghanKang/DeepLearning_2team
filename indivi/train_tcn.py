...
class DrowsinessTCN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.transpose(1, 2) # TCN 요구 규격 (batch_size, channels, seq_len)
        y1 = self.tcn(x)
        out = self.fc(y1[:, :, -1]) # 마지막 시점 출력값
        return out
...
def main():
    ...
    model = DrowsinessTCN(input_size=4, num_channels=[HIDDEN_SIZE, HIDDEN_SIZE, HIDDEN_SIZE], 
    ...
    save_path = "tcn_drowsiness_model.pth"
    
    wandb.init(project="drowsiness-detection", name="TCN")
